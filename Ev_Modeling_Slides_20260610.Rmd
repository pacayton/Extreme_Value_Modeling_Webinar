...
## install.packages(c("tidyverse", "xts", "qrmtools", "rugarch", "knitr", "kableExtra", "moments", "stargazer"))
...
3) Special Cases:
- $GPD(u = 0, \sigma, \xi = 0) \equiv Exp(\lambda = 1/ \sigma)$
...
- Time Series Model to Extract Residuals
- Fitting of the Peaks-over-Thresholds Model
...
```{r, echo = TRUE, warnings = FALSE, message = FALSE, out.width="85%"}
qrmtools::Hill_plot(x = as.numeric(nsresid_psei), 
                    k = c(5, 500))
```
...
(fit_PWM <- qrmtools::fit_GPD_PWM((data_gpd-threshold))) 
...
```{r, echo = TRUE, warnings = FALSE, message = FALSE}
VaR <- qrmtools::VaR_GPDtail(level = 0.99, 
                   threshold = threshold,
                   p.exceed = 150/2881,
                   shape = fit_PWM["shape"],
                   scale = fit_PWM["scale"])
names(VaR) <- "VaR"
ES <- qrmtools::ES_GPDtail(level = 0.99, 
                   threshold = threshold,
                   p.exceed = 150/2881,
                   shape = fit_PWM["shape"],
                   scale = fit_PWM["scale"])
